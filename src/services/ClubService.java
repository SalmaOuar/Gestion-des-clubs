...
public class ClubService implements IDao<Club> {
    ...
    public boolean update(Club o) {
        if (o == null) {
            System.out.println("Erreur : Le club fourni est null !");
            return false;
        }

        if (o.getNom() == null || o.getDescription() == null || o.getDateCreation() == null) {
            System.out.println("Erreur : L'un des champs du club est null !");
            return false;
        }
    ...
    public List<Club> findByNomLike(String nom) {
        List<Club> clubs = new ArrayList<>();
        String req = "SELECT * FROM Club WHERE nom LIKE ?";
        try {
            PreparedStatement ps = connexion.getCn().prepareStatement(req);
            ps.setString(1, "%" + nom + "%");
            ...
            System.out.println("Erreur findByNomLike : " + ex.getMessage());
        }
        return clubs;
    }
